...
library(igraph)
com2cand = read.table("Com2Cand.txt",sep="\t",col.names=c("CommitteeID","CandidateID","Date","Amount"),fill = TRUE,header=FALSE)

print(" Loading Complete")

#Stripping the Date and storing the first 4 characters which signifies year of contribution
com2cand$Date <- substr(com2cand$Date, 1, 4)

#Loading into data frame
df <- data.frame(com2cand)
...
for (i in 1:length(groupByYear)) {
  result[[i]] = cbind(groupByYear[[i]]$CommitteeID, groupByYear[[i]]$CandidateID, groupByYear[[i]]$Amount)
}
...
{	
  ...
  condition1 <- ((y[,3]<=1000))
  condition2 <- ((y[,3]>1000)&(y[,3]<=50000))
  condition3 <- ((y[,3]>50000)&(y[,3]<=1000000))
  condition4 <- ((y[,3]>1000000)&(y[,3]<=10000000))
  condition5 <- ((y[,3]>10000000)&(y[,3]<=30000000))
  condition6 <- ((y[,3]>30000000)&(y[,3]<=50000000))
  condition7 <- ((y[,3]>50000000))
  ...
  y[,3] <- with(y,ifelse(condition6,'v', X3))
...
for( i in 1:length(groupByYear)){
  fileName = paste("dataCom2Cand_",toString(i), ".csv",sep="") 
